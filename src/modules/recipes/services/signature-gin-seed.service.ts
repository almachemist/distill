...
interface SignatureGinIngredient {
  recipe: string
  step: string
  item: string
  qty: number
...
export class SignatureGinSeedService {
  ...
  async seedSignatureGinData(): Promise<{ itemsCreated: number; itemsUpdated: number; recipeCreated: boolean }> {
    ...
    const items: SignatureGinItem[] = [
      { name: 'Ethanol 80.6%', default_uom: 'L', is_alcohol: true, abv_pct: 80.6 },
      ...
      { name: 'Lavender', default_uom: 'g', is_alcohol: false }
    ]
    ...
    const recipeName = 'Signature Dry Gin (Traditional)'
    const ingredients: SignatureGinIngredient[] = [
      { recipe: recipeName, step: 'maceration', item: 'Ethanol 80.6%', qty: 258, uom: 'L', notes: 'Pre-proof charge @ 80.6% ABV' },
      ...
      { recipe: recipeName, step: 'maceration', item: 'Orange peel', qty: 560, uom: 'g' },
      { recipe: recipeName, step: 'maceration', item: 'Lemon peel', qty: 560, uom: 'g' },
      ...
      { recipe: recipeName, step: 'maceration', item: 'Lavender', qty: 40, uom: 'g' },
      { recipe: recipeName, step: 'proofing', item: 'Water', qty: 237, uom: 'L', notes: 'Add to achieve 42% final ABV' }
    ]
    ...
    if (existingRecipe) {
      ...
      await this.supabase
        .from('recipes')
        .update({
          description: 'A timeless and elegant London Dry style gin that showcases the perfect balance of traditional botanicals. This classic expression features juniper-forward character complemented by citrus notes from orange and lemon peel, warm spices from coriander and cardamom, and floral touches from lavender. The addition of Australian macadamia adds a subtle creamy texture and nuttiness that sets this gin apart. A sophisticated choice for gin purists who appreciate refined balance and complexity.',
          notes: 'Signature Dry Gin with baseline 495L @ 42% ABV. LAL: 207.95L from 258L @ 80.6%'
        })
        .eq('id', recipeId)
